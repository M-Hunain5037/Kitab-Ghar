...
const uploadFileToGitHub = async (fileContent, filePath) => {
  ...
  const repo = 'M-Hunain5037/BookDatabase'; // Your GitHub repository
...
if (authors) {
  ...
  dbCredentials.query(query, queryParams, (err, results) => {
    if (err) {
...
app.get('/api/books/pending', verifyToken, (req, res) => {
  console.log('Fetching pending books data from the database...');
...
app.post('/api/auth/register', async (req, res) => {
  const { name, email, password, role = 'user' } = req.body; // Use name instead of fullName
...
app.post('/api/auth/google-login', async (req, res) => {
  try {
    ...
    const checkUserQuery = 'SELECT * FROM users WHERE email = ?';
    dbCredentials.query(checkUserQuery, [email], async (err, results) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ message: 'Database error' });
      }
      ...
      if (results.length === 0) {
        ...
        dbCredentials.query(addUserQuery, [fullName, email, role || 'user'], (err, result) => {
          if (err) {
            console.error('Error inserting user:', err);
            return res.status(500).json({ message: 'Error inserting user' });
          }
...
app.get('/api/users/:userId/profile', verifyToken, (req, res) => {
  ...
  dbCredentials.query(userQuery, [userId], (err, userResult) => {
    if (err) {
      return res.status(500).json({ message: 'Database error', error: err });
    }
    ...
    dbCredentials.query(booksQuery, [userId], (err, booksResult) => {
      if (err) {
        return res.status(500).json({ message: 'Database error', error: err });
      }
